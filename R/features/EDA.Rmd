...
Intraclass Correlation Coefficient (ICC) – Recommended

Since our goal is redundancy / agreement across models, ICC is statistically principled.

    ICC3 ≈ 1 → models give the same value → redundant

    ICC3 ≈ 0 → no agreement → informative difference

Conceptually:

“How much of the variance is due to the feature itself rather than the model?”
...
Treat model as a factor and compute:
    ...
    η² ≈ 0 → redundant across models
    η² < 0.01 → negligible
    0.01–0.06 → small
    0.06 → meaningful
    η² large → strong model effect

This answers:

“How much variance is explained by the model choice?”
...
print(eta2_summary[which(eta2_summary$eta2 <0.02 | is.na(eta2_summary$eta2)),])
```
