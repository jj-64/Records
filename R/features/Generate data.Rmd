...
The second option is to generate simulated data seperately

Specifiy the number of series per each of the four models.

Indicate the requested series length.

Choose if the series generated are to be normalized when generated (divided by max of each series)
```{r Generate data}
raw_data = generate_series_multiple(n_per_model = 100, T_vals = c(100,150,200), normalized = FALSE)
```

We then execute the feature dataset.
