...
Check for columns 
  ...
  ## column number with NAs
na_col_nb = which(colSums(is.na(feature_matrix)) >0 )
na_col_nb
  ## rows with NAs
feature_matrix[!complete.cases(feature_matrix), na_col_nb ]
  ## how much NAs per column
...
# feature_matrix=feature_matrix[complete.cases(feature_matrix),]
  ...
  #### 4.5 get feature matrix (numerics only)
feature_names <- setdiff(names(feature_matrix), c("Max_logLik","series_id", "label", "series"))
feature_only <- feature_matrix[, colnames(feature_matrix) %in% (feature_names)]
# feature_only <- feature_matrix[, sapply(feature_matrix, is.numeric)]

  #### 4.6 Convert to numeric if needed
feature_only <- feature_only %>% mutate_if(is.factor, as.character) %>% mutate_all(as.numeric)

# Add label
#X$label <- feature_matrix$label
#X$series_id <- feature_matrix$series_id

#--------------- 2. Supervised PCA (sanity check) --------------------------
## the aim is to check if generated dataset is really sepearable and can be well identified
## we will plot individuals and should be clearly clustered
pca <- PCA(
  feature_only,
  scale.unit = TRUE,
  graph = FALSE
...
fviz_nbclust(scale(feature_only), kmeans, method = "wss")
