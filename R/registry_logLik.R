...
register_loglik("DTRW", "all", "cauchy",
                fun = function(data, params) {
                  ...
                  sum(dcauchy(x = diff(data), location =as.numeric(params["location"]) , scale = as.numeric(params["scale"]) , log = TRUE))
                }
)
...
register_loglik( "ynm", "all", "gumbel",
                 fun = function(data, params) {
                   ...
                   ## Check for invalid parameter values (e.g., if params[1] <= 0 or params[2] <= 0 or location <= 0, we return -Inf)
                   if (scale <= 0 || gamma <= 0) { ##
                     return(-Inf)  # Invalid parameters, return a large negative value
                   }

                   ## pdf
                   pdf=function(x,par) {VGAM::dgumbel(x, location=par$location, scale=par$scale, log = TRUE)}
                   cdf=function(x,par) {VGAM::pgumbel(x, location=par$location, scale=par$scale, log = TRUE)}

                   # Compute the terms of the likelihood function, while checking for potential issues like negative log arguments
                   s1 = sum( tvec * log(gamma) )
                   if (is.nan(s1) || !is.finite(s1)) return(-Inf)

                   s2 = sum( (gamma^tvec - 1) * cdf(x, par=list(location= location, scale= scale) ) )
                   if (is.nan(s2) || !is.finite(s2)) return(-Inf)

                   s3 = sum(pdf(x, par=list(location= location, scale= scale)))
                   if (is.nan(s3) || !is.finite(s3)) return(-Inf)
