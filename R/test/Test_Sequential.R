...
DT_CLYD = function(X, alpha=0.05, RSq = 0.8, warmup = 2, K=NULL){
  ...
  ## LDM
  if(dec=="no"){ dec=test_ldm_trend(X, alpha=alpha, RSq = RSq)$decision }
  ...
  ##DTRW
  if(dec=="no"){ dec=test_dtrw_increment(X, alpha=alpha)$decision }
...
DT_CLDY = function(X, alpha=0.05, RSq = 0.8, warmup = 2, K=NULL){
  ...
  ## LDM
  if(dec=="no"){
    dec=test_ldm_trend(X, alpha=alpha, RSq = RSq)$decision }
  if(dec!="no"){    return( score=3)}

  ##DTRW
  if(dec=="no"){
    dec=test_dtrw_increment(X, alpha=alpha)$decision
  }
  if(dec!="no"){    return( score=2)}
