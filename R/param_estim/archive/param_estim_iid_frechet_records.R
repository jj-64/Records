...
## file name if saving
file_name = paste0("Simulation_iid_Rn_overT_frechet_shape=",shape_obs,"_scale=", scale_obs, ".xlsx")
###########################  Simulation  ###########################
...
for (T_val in T_values) {
  ...
  while(length(no)>=1){ #while (sim <= simulation){
    ...
    no=which(Theo_var[,1]<0 |Theo_var[,2]<0 |Theo_var[,3]<0  |Emp_var[,1]<0 |Emp_var[,2]<0 |Emp_var[,3]<0 )
  }
