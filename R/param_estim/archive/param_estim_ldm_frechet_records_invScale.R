...
Plot_auto_var = function(matrice = sum_matrices, file_name = "plot_variance.png"){
  ...
  Emp_gamma_var <- sapply(matrice, function(x) x["AVG_Emp_std", "theta"])
  Emp_A_var <- sapply(matrice, function(x) x["AVG_Emp_std", "scale"])
  Emp_alpha_var <- sapply(matrice, function(x) x["AVG_Emp_std", "shape"])

  Theo_gamma_var <- sapply(matrice, function(x) x["AVG_Theo_std", "theta"])
  Theo_A_var <- sapply(matrice, function(x) x["AVG_Theo_std", "scale"])
  Theo_alpha_var <- sapply(matrice, function(x) x["AVG_Theo_std", "shape"])
...
trend_values <- seq(0.25, 0.75, by = 0.05)
...
## file name if saving
file_name = paste0("Simulation_ldm_Rn_frechet_shape=",shape_obs,"_scale_inv=", scale_obs_inv, ".xlsx")
out_path = "data/param_est/ldm/frechet/records/"
...
for (T_val in T_values) {
  ...
  sum_matrices <- list()
    ...
    while(length(no)>=1){ #while (sim <= simulation){
      for(sim in no){
        ...
        #y= LDM_series_Frechet(T_val, theta= trend, shape = shape_obs, scale = scale_obs)
        ...
        if(info == "all"){
          logLik_fun_rec <- loglik_registry[["LDM"]][["all"]][["frechet_inv_scale"]]
          ...
          # MLE_C = nlminb(start = c(theta=0.01, shape = 0.01, scale=0.01),
          #                objective = function(params) - logLik_fun_rec(data = data_rec, params),
          #                lower = c(theta=0.01, shape = 0.01, scale=0.01),
          #                upper = c(theta= min(R/L)-1e-10, shape = 5, scale=5)
          # )
          # if(inherits(MLE_C, "try_error")){
          #   sim=sim-1
          #   next;
          # }
          MLE_C = estimate_model_mle(logLik_fun_rec, obs_type = "records", data = data_rec,
                                     lower_bounds=c(theta=0.01, shape = 0.01, scale=0.01),
                                     upper_bounds = c(theta= min(R/L)-1e-10, shape = 5, scale=5),
                                     start_values = c(
                                       theta = 0.01,
                                       shape = 0.01,
                                       scale =0.01
        ...
        ### Empirical variance
        if(info == "all"){
          # var_fun = var_logLik_records("LDM", "all","frechet", "theta")
          # Emp_var[sim,"theta"] = var_fun(data = y, params=as.list(MLE_C[1:3]) )
          #
          # var_fun = var_logLik_records("LDM", "all","frechet", "scale")
          # Emp_var[sim,"scale"] = var_fun(data = y, params=as.list(MLE_C[1:3]))
          #
          # var_fun = var_logLik_records("LDM", "all","frechet", "shape")
          # Emp_var[sim,"shape"] = var_fun(data = y, params=as.list(MLE_C[1:3]))

          var_fun = var_logLik_records("LDM", "all","frechet_inv", "all")
          ...
          var_fun = var_logLik_records("LDM", "records","frechet_inv", "all")
          emp = var_fun(data = data_rec, params=list(theta = MLE_C["theta"], scale = MLE_C["scale"], shape = MLE_C["shape"]))
          Emp_var[sim,"theta"] = emp[["var_theta"]]
          Emp_var[sim,"scale"] = emp[["var_scale"]]
          Emp_var[sim,"shape"] = emp[["var_shape"]]
        }
        ...
        if(info == "all"){
          #
          # var_fun = var_logLik_records("LDM", "all","frechet", "theta")
          # Theo_var[sim,"theta"] = var_fun(data = y, params=list(theta = trend, scale = scale_obs, shape = shape_obs))
          #
          # var_fun = var_logLik_records("LDM", "all","frechet", "scale")
          # Theo_var[sim,"scale"] = var_fun(data = y, params=list(theta = trend, scale = scale_obs, shape = shape_obs))
          #
          # var_fun = var_logLik_records("LDM", "all","frechet", "shape")
          # Theo_var[sim,"shape"] = var_fun(data = y, params=list(theta = trend, scale = scale_obs, shape = shape_obs))
          var_fun = var_logLik_records("LDM", "all","frechet_inv", "all")
  ...
  #Add to EXCEL
  # if(save){
  #   addWorksheet(wb, c(paste0("T=", T_val)))
  #   writeData(wb,  c(paste0("T=", T_val)) , list_to_df(sum_matrices, var="theta"), rowNames = TRUE , colNames = TRUE)
  # }

  ## Saving
  if(save_details){
    # Add each matrix to a new sheet
    addWorksheet(wb, paste0("T=", T_val,"Sum_", trend))
    writeData(wb, sheet = paste0("T=", T_val,"Sum_", trend), sum_matrix)

    addWorksheet(wb, paste0("T=", T_val,"TheoVar_", trend))
    writeData(wb, sheet = paste0("T=", T_val,"TheoVar_", trend), Theo_var)

    addWorksheet(wb, paste0("T=", T_val,"EmpVar_", trend))
    writeData(wb, sheet = paste0("T=", T_val,"EmpVar_", trend), Emp_var)

    addWorksheet(wb, paste0("T=", T_val,"params_", trend))
    writeData(wb, sheet = paste0("T=", T_val,"params_", trend), params)

    addWorksheet(wb, paste0("T=", T_val,"LogL_", trend))
    writeData(wb, sheet = paste0("T=", T_val,"LogL_", trend), LogL)
  }
}
...
if(save_plot){
  ...
  ggsave(paste0(out_path,"theta_std.png"), plots$std_dev$theta, width = 6, height = 4)
  ...
  ggsave(paste0(out_path,"shape_std.png"), plots$std_dev$shape, width = 6, height = 4)
  ...
  ggsave(paste0(out_path,"scale_std.png"), plots$std_dev$scale, width = 6, height = 4)
