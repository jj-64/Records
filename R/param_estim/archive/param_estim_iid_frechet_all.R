...
Plot_auto = function(variable_name, matrice = sum_matrices, file_name = "plot_bias.png"){
  ...
  A_bias <- sapply(matrice, function(x) x[variable_name, "AHat"])
  shape_bias <- sapply(matrice, function(x) x[variable_name, "shapeHat"])
...
Plot_auto_var = function(matrice = sum_matrices, file_name = "plot_variance.png"){
  ...
  Emp_A_var <- sapply(matrice, function(x) x["AVG_Emp_std", "AHat"])
  Emp_shape_var <- sapply(matrice, function(x) x["AVG_Emp_std", "shapeHat"])

  Theo_gamma_var <- sapply(matrice, function(x) x["AVG_Theo_std", "gammaHat"])
  Theo_A_var <- sapply(matrice, function(x) x["AVG_Theo_std", "AHat"])
  Theo_shape_var <- sapply(matrice, function(x) x["AVG_Theo_std", "shapeHat"])
  ...
  # Add Empirical and Theoretical Variance for AHat
...
## Frechet parameters
scale_obs = 1
scale_inv_obs=1/scale_obs  ## 1/scale
shape_obs=2  ## shape
...
## file name if saving
file_name = paste0("Simulation_iid_Xt_overT_frechet_shape=",shape_obs,"_scale=", scale_obs, ".xlsx")
###########################  Simulation  ###########################
...
for (T_val in T_values) {
  ...
  while(length(no)>=1){ #while (sim <= simulation){
    for(sim in no){
      ...
      y=VGAM::rfrechet(n=T_val,location = 0, shape=shape_obs,scale = scale_obs)
      R=rec_values(y)
      L = rec_times(y)
      ...
      if(info == "all"){
        logLik_fun_rec <- loglik_registry[["iid"]][["all"]][["frechet"]]
        ...
        logLik_fun_rec <- loglik_registry[["iid"]][["records"]][["frechet"]]
        ## Records Data
        ...
        MLE_C = estimate_model_mle(logLik_fun_rec, obs_type = "records", data = data_rec,
                                   lower_bounds=c(location=(min(Rn)-1e-6), shape = 0.01, scale=0.01),
                                   upper_bounds = c(location=(min(Rn)-1e-6), shape = 10, scale=10),
                                   start_values = c(location=(min(Rn)-1e-6), shape = 0.01, scale=0.01)
        )
      ...
      if(info == "all"){
        var_fun = var_logLik_records("iid", "all","frechet", "scale")
        Theo_var[sim,2] = var_fun(data = y, params=list(shape = shape_obs, scale = scale_obs )) ## var of inverted scale
        var_fun = var_logLik_records("iid", "all","frechet", "shape")
        Theo_var[sim,3] = var_fun(data = y, params=list(shape = shape_obs, scale = scale_obs))
      } else{
        var_fun = var_logLik_records("iid", "records","frechet", "scale")
  ...
  for (sim in 1:simulation) {
    ...
    ## CI A
    bound_A = bounds(  params[sim,"AHat"] ,z=z,Theo_var[sim,"A"] )
...
if(save){
  ## Save the workbook
  saveWorkbook(wb, paste0("data/param_est/iid/frechet/all/",file_name), overwrite = TRUE)
}
