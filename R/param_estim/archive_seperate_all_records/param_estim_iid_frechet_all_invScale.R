...
Plot_auto_var = function(matrice = sum_matrices, file_name = "plot_variance.png"){
  ...
  Emp_shape_var <- sapply(matrice, function(x) x["AVG_Emp_var", "shape"])

  Theo_gamma_var <- sapply(matrice, function(x) x["AVG_Theo_var", "gamma"])
  Theo_A_var <- sapply(matrice, function(x) x["AVG_Theo_var", "scale"])
  Theo_shape_var <- sapply(matrice, function(x) x["AVG_Theo_var", "shape"])
...
## Frechet parameters
scale_obs = 2
scale_obs_inv=1/scale_obs  ## 1/scale this is A
shape_obs=1  ## shape
...
## file name if saving
file_name = paste0("Simulation_iid_Xt_overT_frechet_shape=",shape_obs,"_scale=", scale_obs_inv, ".xlsx")
out_path = "data/param_est/iid/frechet_inv_scale/all/"
###########################  Simulation  ###########################
  ...
  while(length(no)>=1){ #while (sim <= simulation){
    for(sim in no){
      ...
      if(info == "all"){
        ...
        logLik_fun_rec <- loglik_registry[["iid"]][["records"]][["frechet_inv_scale_explicit"]]
        ## Records Data
      ...
      params[sim,"gamma"] <- 1  ## Gamma always 1 in iid
