...
## file name if saving
file_name = paste0("Simulation_DTRW_Rn_overT_norm_sd=",sd_obs,".xlsx")
###########################  Simulation  ###########################
  ...
  while(length(no)>=1){ #while (sim <= simulation){
    for(sim in no){
      ...
      if(info == "all"){
        ...
        #                            lower_bounds=c(mean=0, sd = 1),
        #                            upper_bounds = c(mean=0, sd = 20),
        #                            start_values = c(mean = 0, sd = 11) )
        # MLE_C = c(MLE_C$par^2,MLE_C$objective ) ## return mean, sd, objective value
...
if(save){
  ## Save the workbook
  saveWorkbook(wb, paste0("data/param_est/DTRW/norm/records/",file_name), overwrite = TRUE)
}
