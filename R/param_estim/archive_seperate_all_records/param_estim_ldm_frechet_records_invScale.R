...
## Series length
#T_val=50
T_values <- c(25, 50, 100, 200)

# Define the sequence of theta values
trend_values <- seq(0.05, 0.50, by = 0.05)
...
out_path = "data/param_est/ldm/frechet_inv_scale/records/"
      ...
      for(sim in no){
        ...
        if(info == "all"){
          ...
          MLE_C = estimate_model_mle(logLik_fun_rec, obs_type = "records", data = data_rec,
                                     ...
                                     upper_bounds = c(theta= min(R/L)-1e-10, shape = 100, scale=100),
                                     start_values = c(
                                       theta = 0.01,
                                       shape = 0.01,
                                       scale =0.01
