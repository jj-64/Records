...
## file name if saving
file_name = paste0("Simulation_ldm_Xt_gumbel_location=",location_obs,"_scale=", scale_obs, ".xlsx")
out_path = "data/param_est/ldm/gumbel/all/"
    ...
    while(length(no)>=1){ #while (sim <= simulation){
      for(sim in no){
        ...
        if(info == "all"){
          ...
          # MLE_C = nlminb(start = c(theta=0.01, location = 0.01, scale=0.01),
          #                objective = function(params) - logLik_fun_rec(data = data_rec, params),
          #                lower = c(theta=0.01, location = 0.01, scale=0.01),
          #                upper = c(theta= min(R/L)-1e-10, location = 5, scale=5)
          # )
          ...
          MLE_C = c(MLE_C$par, "objective" = - MLE_C$objective ) ## return location, location, scale, objective value
          }
        ...
        ### Empirical variance
        if(info == "all"){
          # var_fun = var_logLik_records("LDM", "all","gumbel", "theta")
          # Emp_var[sim,"theta"] = var_fun(data = y, params=as.list(MLE_C[1:3]) )
          #
          # var_fun = var_logLik_records("LDM", "all","gumbel", "scale")
          # Emp_var[sim,"scale"] = var_fun(data = y, params=as.list(MLE_C[1:3]))
          #
          # var_fun = var_logLik_records("LDM", "all","gumbel", "location")
          # Emp_var[sim,"location"] = var_fun(data = y, params=as.list(MLE_C[1:3]))
          ...
          emp = var_fun(data = data_rec, params=list(theta = MLE_C["theta"], scale = MLE_C["scale"], location = MLE_C["location"]))
